...
impl DaemonClient {
    ...
    /// `POST <path>` with a JSON body, decoded as `T`. The same 401-then-
    /// reread-and-retry-once contract as [`DaemonClient::get`] — a spending
    /// or destructive action (extend, terminate) is exactly the request a
    /// daemon restart must not silently drop.
