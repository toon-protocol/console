...
//! opens [`ConfirmState`] (via [`activate`]), and only that modal's own two
//! distinct keys (`y` then `Enter`) produce the command. See
//! `widgets::confirm` and `views::account::handle_key`'s check at its top.
...
use crate::widgets::confirm::ConfirmState;
...
pub enum Target {
    ...
    Mnemonic,
    ImportSubmit,
    /// Opens the confirm modal; the modal's own `y`-then-`Enter` is what
...
pub fn activate(
    ...
    match target {
        ...
        Target::Publish => {
            state
                .confirm
                .open("Publish the Chain Seed", confirm_body(status));
            Command::None
        }
...
mod tests {
    ...
    fn one_keypress_alone_can_never_publish_end_to_end() {
        ...
        assert!(state.confirm.is_open());
        let outcome = state.confirm.handle_key(key(KeyCode::Enter));
        assert_eq!(outcome, crate::widgets::confirm::ConfirmOutcome::Handled);
        assert!(
            state.confirm.is_open(),
            "an unarmed Enter must not confirm or close the modal"
        );
    }
