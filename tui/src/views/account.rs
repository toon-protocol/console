...
pub fn handle_key(
    ...
    // the two keys it takes to confirm (`y`, then `Enter`) are never mixed
    // up with this view's own cursor movement.
    if state.chain_seed.confirm.is_open() {
        return Some(match state.chain_seed.confirm.handle_key(key) {
            ConfirmOutcome::Confirmed => Command::PublishChainSeed,
            ConfirmOutcome::Handled => Command::None,
        });
    }
...
pub fn draw(
    ...
    if status.signed_in {
        ...
        state.chain_seed.confirm.draw(frame, area);
    } else {
...
mod tests {
    ...
    fn renders_the_confirm_modal_over_the_whole_account_view() {
        ...
        assert!(state.chain_seed.confirm.is_open());
        let output = render(|frame| {
            draw(
                frame,
                frame.area(),
        ...
        assert!(output.contains("y to arm"), "shows the two-key dance");
    }
}
